\section{Methods}\label{sec:methods}\label{sec:meth-start}
...
\subsection{Feature Engineering and Selection Implementation}\label{subsec:feature_engineering}
...
\subsubsection{Cross-Domain Recursive Feature Elimination (RFE)}\label{sec:methods-rfe}\label{subsubsec:rfe_implementation}
To instantiate the abstract operator $\pi_{\mathrm{RFE}}$ from Section~\ref{subsubsec:cross-domain-rfe}, PANDA employs a concrete cross-domain RFE procedure on the source cohort. The goal is to identify a compact subset $\rfeselectedfeatures$ that remains available in the target schema while preserving strong source-domain discrimination under TabPFN-based inference.

The RFE procedure relies on permutation importance computed on cross-validated source-domain folds. Specifically, for each feature $j$ we compute its importance as the degradation in AUC when its values are randomly shuffled within the validation folds of the \emph{source} dataset. Let $D^{\text{(cv)}}_{\text{S,val}}$ denote the union of held-out folds in a $K$-fold cross-validation on $(X_{\mathrm{S}}, y_{\mathrm{S}})$. We define:
\begin{equation}
    \label{eq:permutation-importance}
    I_j = L_{\text{AUC}}\!\bigl(D^{\text{(cv)}}_{\text{S,val}}\bigr)
      - \frac{1}{K} \sum_{k=1}^K
        L_{\text{AUC}}\!\bigl(D^{\text{(cv)},(j,\text{shuffled})}_{\text{S,val},k}\bigr),
\end{equation}
where $D^{\text{(cv)},(j,\text{shuffled})}_{\text{S,val},k}$ is obtained by independently permuting feature $j$ in the $k$-th validation fold while keeping all other features and labels fixed.

Importantly, permutation importance and CEI are computed \emph{only} on source-domain cross-validation folds; the external validation cohort and the target hospital are never used during feature selection, avoiding any leakage of test-set information into the RFE procedure.

\paragraph{Algorithm \ref{alg:cross_domain_rfe}: Cross-Domain Recursive Feature Elimination (RFE) with CEI Optimization}
\label{alg:cross_domain_rfe}
\begin{enumerate}
  \item \textbf{Input:} source data $(\sourcedatafeatures, \sourcedatalabels)$; target feature count range $[k_{\min}, k_{\max}]$.
  \item Initialize the TabPFN wrapper (permutation importance enabled) and records $\mathcal{M} \leftarrow \emptyset$.
  \item For each $k$ from $k_{\max}$ down to $k_{\min}$:
  \begin{enumerate}
    \item Configure sklearn RFE: estimator=TabPFNWrapper, $n\_features\_to\_select=k$, step=1.
    \item Fit RFE to obtain $(\rfeset_k, \text{ranking}_k)$ on $(\sourcedatafeatures, \sourcedatalabels)$.
    \item Evaluate $\metrick(k) = \text{CV-AUC}(\rfeset_k)$ with $\numfolds$-fold CV.
    \item Compute CEI components $\rfeperformance(k)$, $\rfeefficiency(k)$, $\rfestability(k)$, $\rfesimplicity(k)$.
    \item Compute $\text{CEI}(k)$ from the weighted sum of the four components.
    \item Append $(k, \rfeset_k, \text{CEI}(k))$ to $\mathcal{M}$.
  \end{enumerate}
  \item Select $\rfecurrentdim^* = \argmax_k \text{CEI}(k)$ and set $\rfeselectedfeatures \leftarrow \rfeset_{\rfecurrentdim^*}$.
  \item \textbf{Output:} optimal feature subset $\rfeselectedfeatures$.
\end{enumerate}
...
Once the feature set is fixed, PANDA leverages TabPFN's internal ensemble mechanism to handle distribution shifts in feature scaling and encoding. This step primarily addresses the variance introduced by different preprocessing choices and minor covariate shifts in the marginal feature distributions. This mechanism can be viewed as a form of test-time augmentation for tabular data.

We instantiate four preprocessing configurations that differ in their distributional transform and categorical-feature handling (Fig.~\ref{fig:model_details}). Specifically:
\begin{enumerate}
    \item \textbf{No-transform + numeric categoricals:} no distributional transformation and treating categorical features as numeric.
    \item \textbf{Quantile + ordinal:} quantile transformation of continuous features and ordinal encoding of categorical variables.
    \item \textbf{No-transform + ordinal:} no distributional transformation and ordinal encoding of categorical variables.
    \item \textbf{Quantile + numeric categoricals:} quantile transformation of continuous features and treating categorical features as numeric.
\end{enumerate}
For each configuration, TabPFN applies random feature-order rotations and runs eight parallel forward passes, yielding a $4 \times 8 = 32$-member ensemble that is subsequently aggregated. Each branch is applied to both support (train) and query (test) sets simultaneously, ensuring a consistent mapping.

\subsection{Foundation Model Integration Mechanism}\label{sec:methods-tabpfn}\label{subsec:foundation_model_integration}
...
\begin{itemize}
    \item \textbf{Preprocessing and Encoding:} Features undergo preprocessing such as quantile and power transformations. Categorical variables are primarily handled via ordinal encoding, converting categories to integers, or processed as mixed types, rather than relying solely on traditional embedding lookup tables.
    ...
    \item \textbf{Linear Projection:} Each preprocessed feature is linearly projected into the embedding space.
    \begin{equation}
        \label{eq:feature-embedding}
        \mathbf{e}^{(j)} = \text{Linear}(\text{Preprocess}(\featurevec^{(j)}))
    \end{equation}
\end{itemize}

Distinct from architectures that concatenate all features into a single vector ($\samplesembed = \text{MLP}(\text{Concat}(\dots))$), TabPFN maintains a per-feature representation. The Transformer architecture utilizes attention mechanisms that operate across both features and samples, allowing the model to learn complex interactions between specific covariates while attending to relevant patients within the context window.

\subsubsection{Ensemble Construction and Inference}\label{sec:methods-ensemble}
...
\paragraph{Algorithm \ref{alg:tabpfn_inference}: PANDA Inference with TabPFN Backbone}\label{alg:tabpfn_inference}
\begin{enumerate}
  \item \textbf{Input:} query $\querypatient$, context $\mathcal{D}_{\text{train}}$, ensemble size $N=32$.
  \item Initialize an empty list of logits.
  \item For $i=1$ to $N$:
  \begin{enumerate}
    \item Sample preprocessing $P \sim \preprocessingbranches$ and a context subset $\mathcal{D}_i \subset \mathcal{D}_{\text{train}}$.
    \item Apply the branch: $(\mathbf{x}'_q, \mathcal{D}'_i) \leftarrow P(\querypatient), P(\mathcal{D}_i)$.
    \item Serialize tokens $\mathbf{E} \leftarrow \text{Serialize}(\mathbf{x}'_q, \mathcal{D}'_i)$.
    \item Forward through the Transformer to get $\mathbf{z}$; compute $\ceilogit = \text{ClassifierHead}(\mathbf{z})$.
    \item Append $\ceilogit$ to the logits list.
  \end{enumerate}
  \item Compute $\hat{\labelval} = \text{Softmax}(\text{Mean}(\text{Logits}) / \temperature)$.
  \item \textbf{Output:} malignancy probability $\hat{\labelval}$.
\end{enumerate}

\subsubsection{Transfer Component Analysis (TCA)}\label{sec:methods-tca}\label{subsubsec:tca_implementation}
...
\paragraph{Algorithm \ref{alg:tca_optimization}: Transfer Component Analysis (TCA) Optimization}
\label{alg:tca_optimization}
\begin{enumerate}
  \item \textbf{Input:} source features $\sourcedatafeatures$, target features $\targetfeatureschema$, kernel $k(\cdot,\cdot)$, target dimension $\tcaprojectdim$, regularization $\regularizationparam$.
  \item Build kernel blocks $\kernelmatrix_{SS}, \kernelmatrix_{ST}, \kernelmatrix_{TT}$ and assemble
        $\kernelmatrix = \begin{bmatrix} \kernelmatrix_{SS} & \kernelmatrix_{ST} \\ \kernelmatrix_{ST}^\top & \kernelmatrix_{TT} \end{bmatrix}$.
  \item Construct the MMD matrix $\mmdmatrix$ with entries $\frac{1}{\sourcedatasize^2}$ (source-source), $\frac{1}{\targetdatasize^2}$ (target-target), and $-\frac{1}{\sourcedatasize \targetdatasize}$ (cross-domain).
  \item Construct the centering matrix $\centeringmatrix = \identitymatrix - \frac{1}{\sourcedatasize+\targetdatasize}\onesvec\onesvec^\top$.
  \item Form $\mathbf{A} = \identitymatrix + \regularizationparam \kernelmatrix \mmdmatrix \kernelmatrix$ and $\mathbf{B} = \kernelmatrix \centeringmatrix \kernelmatrix$.
  \item Solve the generalized eigenproblem for $\mathbf{A}^{-1}\mathbf{B}$; take the top $\tcaprojectdim$ eigenvectors as $\tcaprojectionmatrix$.
  \item \textbf{Output:} projection matrix $\tcaprojectionmatrix$.
\end{enumerate}
...
\subsection{Experimental Configuration}\label{subsec:experimental_config}
...
\begin{itemize}
    ...
    \item \textbf{PKUPH model}: A regression model specifically developed for Chinese populations \cite{li2011development}: $\text{Prob} = \sigma(-4.50 + 0.07 \cdot \text{Age} + 0.68 \cdot \text{Diameter} + 0.74 \cdot \text{Spiculation} + 1.27 \cdot \text{FamilyHx} - 1.62 \cdot \text{Calcification} - 1.41 \cdot \text{Boundary})$.
...
We do not include deep tabular architectures such as TabNet, TabTransformer, SAINT, or FT-Transformer as baselines in our main experiments. Large-scale benchmarking studies on tabular data have shown that, on small heterogeneous datasets with fewer than 1,000 samples, well-regularized gradient-boosted trees typically outperform deep tabular networks, which are highly sensitive to hyperparameters and require extensive tuning and GPU resources to reach competitive performance \cite{gorishniy2021revisiting,borisov2022deep}. In our setting, each hospital cohort contains only 295 patients with 63 candidate features and a moderately imbalanced outcome distribution (approximately a 1:1.78 ratio between normal and abnormal cases). This places us in the small-sample, high-variance regime where prior studies report that deep tabular models are unstable and often underperform tree ensembles. Guided by these findings, we treat tuned tree ensembles and our pre-trained tabular foundation model as the most informative baselines for this regime and allocate our experimental budget to robust cross-validation and ablations within the PANDA pipeline rather than to underpowered and potentially misleading deep-tabular comparisons.
