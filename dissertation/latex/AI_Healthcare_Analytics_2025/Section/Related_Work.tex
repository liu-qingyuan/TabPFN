...
Standard feature selection methods treat all samples as exchangeable, implicitly assuming
that feature-importance rankings are identical across domains. Domain-aware approaches
...
meaningful biomarkers. PANDA adopts this philosophy in a pragmatic way: TabPFN serves
as a strong scoring model, whereas feature elimination is guided jointly by
source-site performance and cross-site stability, yielding a compact
...
alignment: rather than matching full distributions in a high-dimensional space, one
first discards variables whose predictive contributions are strongly domain-specific
...
is both statistically and operationally simpler. PANDA instantiates
...
thereby coupling representation learning with domain-aware feature governance. As summarized in Table~\ref{tab:fs_methods}, recent feature selection methods for small, imbalanced, high-dimensional biomedical tabular data span classical wrappers, sparsity-regularized linear models, graph-based approaches, and deep feature screening.
...
volumes, providing richer representations than clinical risk scores but introducing
...
reduces scanner effects but requires batch labels and can blur subtle lesions~\cite{hellin2024unraveling,
...
typically report AUCs in the 0.75--0.90 range, yet these values rarely carry over
to new scanners or hospitals. External validations on LIDC-IDRI, LUNA16, and NLST
repeatedly report double-digit AUC drops when models are deployed to scanners with different
...
zech_variable_2018,koch2024distribution}. These failures indicate that radiomics
alone does not guarantee transportability and that alignment plus feature vetting are
required before cross-hospital deployment.

Representative radiomics-based pulmonary nodule malignancy models and their reported generalization behavior across scanners and centers are summarized in Table~\ref{tab:radiomics_models}.

Concrete exemplars reinforce this fragility. The Bayesian Integrated Malignancy
...
LASSO~\cite{garau_external_2020,hellin2024unraveling}. Radiomics therefore provides
