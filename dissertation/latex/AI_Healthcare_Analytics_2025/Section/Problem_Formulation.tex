\section{Problem Formulation}\label{sec:problem-formulation}\label{sec:pf-start}
...
\subsubsection{Stage 1: Cross-Domain Recursive Feature Elimination (RFE)}
\label{subsubsec:cross-domain-rfe}

The first stage, $\rfeop$, addresses the feature-mismatch challenge between hospitals. Rather than optimizing feature selection for a single dataset, we seek a subset $\rfeselectedfeatures$ that (i) remains available in the target schema and (ii) yields strong predictive performance on the source domain.

Concretely, $\rfeop$ implements a cross-domain variant of recursive feature elimination. A TabPFN-based permutation-importance estimator scores each feature on source-domain cross-validation folds, and candidate subsets of size $k \in [k_{\min}, k_{\max}]$ are evaluated using a composite evaluation index (CEI) that trades off discrimination, computational efficiency, stability across folds, and parsimony. The maximizer $k^{\ast}$ defines the selected subset $\rfeselectedfeatures$, which then serves as the input to the subsequent schema-alignment and domain-adaptation stages.

The full implementation details of the cross-domain RFE procedure, including the CEI components and the permutation-importance estimator, are provided in Section~\ref{sec:methods-rfe}.
...
\begin{equation}
    \label{eq:domain-adaptation-map}
...
Finally, the classification function $\hypothesis$ processes the adapted features $\featurevec'$. This stage leverages the pre-trained TabPFN as a robust classifier, augmented by an ensemble mechanism to handle predictive uncertainty and distribution shift.
\begin{equation}
    \label{eq:ensemble-hypothesis}
    \hypothesis(\featurevec') = \frac{1}{N} \sum_{i=1}^{N} \activation\left(\frac{\tabpfnfunci{i}(\featurevec')}{\temperature}\right)
\end{equation}
where $\temperature$ is a temperature scaling parameter optimized to calibrate the output probabilities against the prevalence differences between hospitals. The result is a calibrated malignancy probability that is robust to both covariate and concept shifts.

While PANDA is in principle applicable under covariate, concept, and label shift, in this thesis we evaluate these regimes in two distinct settings. In our pulmonary nodule experiment, the primary challenges are covariate and concept shift across hospitals, as discussed in Section~\ref{sec:problem-constraints}, rather than a canonical low-prevalence label-shift scenario. The ability of PANDA to tolerate substantial label shift and prevalence changes is evaluated separately on the TableShift BRFSS Diabetes benchmark, where the positive-class prevalence differs markedly across race-defined domains.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{0.25\textwidth}p{0.25\textwidth}p{0.2\textwidth}p{0.2\textwidth}}
    ...
    \midrule
    Referral Patterns (Cancer Center vs. Screening) & Distribution Shift: $\marginalsourcelabel \neq \marginaltargetlabel$ (dominant in TableShift) & Ensemble Aggregation \& Temperature Scaling & Calibrates posteriors; smooths overconfidence from prior mismatch. \\
    \midrule
...
\subsection{Problem Constraints and Research Scope}\label{sec:problem-constraints}\label{subsec:constraints}
...
\begin{itemize}
    ...
    \item \textbf{Class Imbalance:} In real-world screening settings, the prevalence of the positive class is often low ($\prevalence < 0.3$), which motivates \auc-centric optimization. Although our development cohorts are enriched case-control samples with higher malignancy rates, we retain AUC-focused evaluation criteria to maintain relevance to such low-prevalence deployments.
\end{itemize}

This formalization sets the stage for the specific methodological implementations detailed in Section~\ref{sec:methods} .
\label{sec:pf-end}
