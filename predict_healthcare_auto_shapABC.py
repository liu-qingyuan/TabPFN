...
if __name__ == "__main__":
    # 加载数据集
    print("\nLoading datasets...")
    print("1. Loading AI4healthcare.xlsx (Dataset A)...")
    df_a = pd.read_excel("data/AI4healthcare.xlsx")
    
    print("2. Loading HenanCancerHospital_features63_58.xlsx (Dataset B)...")
    df_b = pd.read_excel("data/HenanCancerHospital_features63_58.xlsx")
    
    print("3. Loading GuangzhouMedicalHospital_features23_no_nan.xlsx (Dataset C)...")
    df_c = pd.read_excel("data/GuangzhouMedicalHospital_features23_no_nan.xlsx")
    
    # 定义数据集和对应的特征（所有特征，除了Label）
    datasets = {
        'A': {
            'data': df_a,
            'features': [col for col in df_a.columns if col != 'Label']
        },
        'B': {
            'data': df_b,
            'features': [col for col in df_b.columns if col != 'Label']
        },
        'C': {
            'data': df_c,
            'features': [col for col in df_c.columns if col != 'Label']
        }
    }
    ...
    for dataset_name, dataset_info in datasets.items():
        ...
        print(f"Dataset {dataset_name} has {len(features)} features")
        ...
        print(f"\nDataset {dataset_name} split (80/20):")
        ...
        # 设置结果目录
        base_path = f'./results_Dataset_{dataset_name}_auto'
        os.makedirs(base_path, exist_ok=True)
        
        # 运行实验
        print(f"\n=== Dataset {dataset_name} Analysis with AutoTabPFN ===")
        ...
        datasets_info.to_csv(f'{base_path}/dataset_info.csv', index=False)
    ...
    for dataset_name in datasets.keys():
        base_path = f'./results_Dataset_{dataset_name}_auto'
        results_file = f'{base_path}/summary_results.csv'
    ...
    if all_results:
        ...
        # 创建汇总目录
        summary_path = './results_summary_ABC'
        os.makedirs(summary_path, exist_ok=True)
        
        # 保存汇总结果
        final_results.to_csv(f'{summary_path}/all_datasets_results.csv', index=False)
        
        # 打印汇总表格
        summary_table = final_results[['Dataset', 'acc', 'auc', 'f1', 'acc_0', 'acc_1', 'time']]
        print("\nResults Summary:")
        print(summary_table.to_string(index=False))
        
        # 创建比较图表
        metrics_to_plot = ['acc', 'auc', 'f1']
        ...
        for metric in metrics_to_plot:
            ...
            plt.bar(x + offset, final_results[metric], width, label=metric.upper())
            multiplier += 1
        
        plt.xlabel('Dataset')
        plt.ylabel('Score')
        plt.title('Performance Comparison Across Datasets')
        plt.xticks(x + width, datasets.keys())
        plt.legend(loc='lower right')
        plt.ylim(0, 1.0)
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        plt.savefig(f'{summary_path}/performance_comparison.png', dpi=300, bbox_inches='tight')
