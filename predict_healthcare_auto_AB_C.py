...
if __name__ == "__main__":
    ...
    # 合并 A 和 B 数据集
    print("\n合并 A (AI4health) 和 B (河南) 数据集...")
    X_combined = pd.concat([X_ai4health, X_henan], axis=0)
    y_combined = pd.concat([y_ai4health, y_henan], axis=0)
    ...
    scores_df_combined = run_experiment(
        ...
        max_time=180, 
        random_state=42,
        base_path='./results_AB_combined'
    )

    # 在合并数据集上训练，在 C 上测试
    print("\n\n=== 在合并数据集 (A+B) 上训练，在 C (广州) 上测试 ===")
    external_metrics, trained_model = train_and_evaluate_on_external(
        ...
        max_time=180, 
        random_state=42,
        base_path='./results_AB_train_C_test'
    )

    # 保存数据集信息
    print("\n=== 数据集信息对比 ===")
    datasets_info = pd.DataFrame({
        'Dataset': ['A (AI4health)', 'B (Henan)', 'C (Guangzhou)', 'A+B Combined'],
    ...
    # 保存数据集信息
    os.makedirs('./results_AB_C_comparison', exist_ok=True)
    datasets_info.to_csv('./results_AB_C_comparison/datasets_info.csv', index=False)
    print("\n数据集信息已保存至: ./results_AB_C_comparison/datasets_info.csv")

    # 汇总所有结果
    summary_results = pd.DataFrame({
        'Metric': ['AUC', 'F1', 'ACC', 'ACC_0', 'ACC_1'],
        'A+B CV': [scores_df_combined['auc'].mean(), 
                  scores_df_combined['f1'].mean(),
                  scores_df_combined['accuracy'].mean(),
                  scores_df_combined['acc_0'].mean(),
                  scores_df_combined['acc_1'].mean()],
        'C (External)': [external_metrics['auc'],
                        external_metrics['f1'],
    ...
    # 保存汇总结果
    summary_results.to_csv('./results_AB_C_comparison/summary_results.csv', index=False)
    print("\n汇总结果已保存至: ./results_AB_C_comparison/summary_results.csv")
    print("\n=== 汇总结果 ===")
