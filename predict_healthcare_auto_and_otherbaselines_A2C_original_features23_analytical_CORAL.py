...
if __name__ == "__main__":
    ...
    logging.info("2. Loading GuangzhouMedicalHospital_features23_no_nan.xlsx (C)...")
    ...
    # 创建结果目录
    os.makedirs('./results_analytical_coral_A2C_original', exist_ok=True)
    ...
    if only_visualize:
        ...
        configs = [
            {
                ...
                'npz_file': './results_analytical_coral_A2C_original/TabPFN-Analytical-CORAL_A_to_C_aligned_features.npz'
            }
        ]
        ...
        for config in configs:
            ...
            # 使用t-SNE可视化对齐前后的分布
            tsne_save_path = f"./results_analytical_coral_A2C_original/TabPFN-Analytical-CORAL_{config['name']}_tsne_detailed.png"
            visualize_tsne(
                X_source=X_source,
                X_target=X_target,
            ...
            # 为前5个特征绘制分布直方图
            hist_save_path = f"./results_analytical_coral_A2C_original/TabPFN-Analytical-CORAL_{config['name']}_histograms.png"
            visualize_feature_histograms(
                X_source=X_source,
                X_target=X_target,
                X_target_aligned=X_target_aligned,
        ...
        for config in coral_configs:
            ...
            metrics = run_coral_adaptation_experiment(
                ...
                base_path='./results_analytical_coral_A2C_original'
            )
        ...
        results_df.to_csv('./results_analytical_coral_A2C_original/all_results.csv', index=False)
        logging.info("Results saved to ./results_analytical_coral_A2C_original/all_results.csv")
        ...
        for idx, config in enumerate(coral_configs):
            ...
            plt.savefig(f'./results_analytical_coral_A2C_original/comparison_{config["name"]}.png', dpi=300)
            plt.close()
            
        logging.info("\nAll results saved to ./results_analytical_coral_A2C_original/ directory")

        # 创建类条件CORAL结果目录
        os.makedirs('./results_class_conditional_coral_A2C_original', exist_ok=True)
        ...
        for config in coral_configs:
            ...
            class_metrics = run_class_conditional_coral_experiment(
                ...
                base_path='./results_class_conditional_coral_A2C_original',
                optimize_decision_threshold=True,
        ...
        class_coral_df.to_csv('./results_class_conditional_coral_A2C_original/class_coral_results.csv', index=False)
        logging.info("类条件CORAL结果保存至 ./results_class_conditional_coral_A2C_original/class_coral_results.csv")
        ...
        for idx, config in enumerate(coral_configs):
            ...
            plt.tight_layout()
            plt.subplots_adjust(top=0.9)
            plt.savefig(f'./results_class_conditional_coral_A2C_original/methods_comparison_{config["name"]}.png', dpi=300)
            plt.close()
        
        logging.info("\n所有类条件CORAL结果保存至 ./results_class_conditional_coral_A2C_original/ 目录")
    ...
    # 处理解析版CORAL的NPZ文件
    npz_files = glob.glob('./results_analytical_coral_A2C_original/*_aligned_features.npz')
    ...
    # 处理类条件CORAL的NPZ文件
    class_coral_npz_files = glob.glob('./results_class_conditional_coral_A2C_original/*_aligned_features.npz')
