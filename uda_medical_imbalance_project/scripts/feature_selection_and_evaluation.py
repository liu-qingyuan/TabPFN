...
配置说明：
- 集成配置：256个estimators（16种基础配置各16个成员，16×16优化集成）
...
class TabPFNWrapper(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, device='cuda', n_estimators=256, softmax_temperature=0.9,
...
def evaluate_feature_performance(X, y, feature_ranking, results_dir):
    ...
    print(f"⏰ 预计总用时: {len(feature_numbers) * 5:.1f}-{len(feature_numbers) * 10:.1f}分钟 (使用256集成成员，4倍于64配置的时间)")
...
def main():
    ...
    # 创建时间戳输出目录 (标注使用58个特征，256个estimators配置)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = project_root / "results" / f"feature_selection_evaluation_256estimators_{timestamp}"
    ...
    try:
        ...
        print(f"   TabPFN配置: 256集成成员 (16种配置 × 16成员)")
    ...
    print("🚀 使用256集成成员（16种配置 × 16成员）获得优化性能")
    print("⏰ 预计用时：15-30分钟 (使用256集成成员，适中GPU计算资源)")
    ...
    print(f"📊 RFE处理: 从AB交集{X.shape[1]}个特征开始，使用256集成成员逐步消除到3个特征")
    ...
    print(f"4. 结果数据行数: {len(results_df)} 行 (从3个特征到58个特征，256集成配置)")
