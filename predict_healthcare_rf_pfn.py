...
def run_experiment(
    ...
    """
    Run RandomForestTabPFN experiment with given parameters
    ...
    exp_name = f"RF-TabPFN-Health-R{random_state}"
    ...
    base_model_final = TabPFNClassifier(device=device)
    final_clf = RandomForestTabPFNClassifier(
        tabpfn=base_model_final,
        random_state=random_state
    )
...
if __name__ == "__main__":
    ...
    # Run experiment with RandomForestTabPFNClassifier
    print("\nRunning experiment with RandomForestTabPFNClassifier...")
    result_df, final_model = run_experiment(
        X=X,
        y=y,
        device='cuda',
        random_state=42,
        base_path='./results/rf_pfn_model'
    )
    ...
    # Save to file
    save_path = './results/rf_pfn_model'
    os.makedirs(save_path, exist_ok=True)
    joblib.dump(best_model_info, f'{save_path}/RF-TabPFN-Health-Model.joblib')
    
    # Save parameters in readable format
    with open(f'{save_path}/RF-TabPFN-Health-Parameters.txt', 'w') as f:
