...
if __name__ == "__main__":
    ...
    if only_visualize:
        ...
        configs = [
            {
                ...
                'X_target': X_henan,
                'y_target': y_henan,
                'npz_file': './results_analytical_coral/TabPFN-Analytical-CORAL_A_to_B_aligned_features.npz'
            },
            {
                'name': 'A_to_C',
                'source_name': 'A_AI4health',
                ...
                'npz_file': './results_analytical_coral/TabPFN-Analytical-CORAL_A_to_C_aligned_features.npz'
            }
        ]
        ...
        for config in configs:
            ...
            # 使用t-SNE可视化对齐前后的分布
            tsne_save_path = f"./results_analytical_coral/TabPFN-Analytical-CORAL_{config['name']}_tsne_detailed.png"
            visualize_tsne(
                X_source=X_source,
                X_target=X_target,
            ...
            # 为前5个特征绘制分布直方图
            hist_save_path = f"./results_analytical_coral/TabPFN-Analytical-CORAL_{config['name']}_histograms.png"
            visualize_feature_histograms(
                X_source=X_source,
                X_target=X_target,
                X_target_aligned=X_target_aligned,
        ...
        results_df.to_csv('./results_analytical_coral/all_results.csv', index=False)
        logging.info("Results saved to ./results_analytical_coral/all_results.csv")
        ...
        for idx, config in enumerate(coral_configs):
            ...
            plt.savefig(f'./results_analytical_coral/comparison_{config["name"]}.png', dpi=300)
            plt.close()
            
        logging.info("\nAll results saved to ./results_analytical_coral/ directory")

        # 创建类条件CORAL结果目录
        os.makedirs('./results_class_conditional_coral', exist_ok=True)
        ...
        class_coral_df.to_csv('./results_class_conditional_coral/class_coral_results.csv', index=False)
        logging.info("类条件CORAL结果保存至 ./results_class_conditional_coral/class_coral_results.csv")
        ...
        for idx, config in enumerate(coral_configs):
            ...
            plt.tight_layout()
            plt.subplots_adjust(top=0.9)
            plt.savefig(f'./results_class_conditional_coral/methods_comparison_{config["name"]}.png', dpi=300)
            plt.close()
        
        logging.info("\n所有类条件CORAL结果保存至 ./results_class_conditional_coral/ 目录")
    ...
    # 处理解析版CORAL的NPZ文件
    npz_files = glob.glob('./results_analytical_coral/*_aligned_features.npz')
    ...
    for npz_file in npz_files:
        ...
        # 从文件名中提取配置信息
        if 'A_to_B' in file_name:
            source_name = 'A_AI4health'
            target_name = 'B_Henan'
            y_source = y_ai4health
            y_target = y_henan
        elif 'A_to_C' in file_name:
            source_name = 'A_AI4health'
            target_name = 'C_Guangzhou'
            y_source = y_ai4health
            y_target = y_guangzhou
    ...
    # 处理类条件CORAL的NPZ文件
    class_coral_npz_files = glob.glob('./results_class_conditional_coral/*_aligned_features.npz')
