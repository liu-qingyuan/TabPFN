...
os.makedirs('./results/best_8_features_results_AutoPostHocEnsemblePredictor', exist_ok=True)
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X_train_scaled), 1):
    ...
    model = AutoPostHocEnsemblePredictor(
        preset="default",
        max_time=60,  # 60 seconds per fold
...
# Save to file
with open('./results/best_8_features_results_AutoPostHocEnsemblePredictor/results.txt', 'w') as f:
    f.write("Best 8 Features Evaluation Results with AutoPostHocEnsemblePredictor\n")
    f.write("=" * 50 + "\n\n")
...
print("\nResults have been saved to: ./results/best_8_features_results_AutoPostHocEnsemblePredictor/results.txt") 